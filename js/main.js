...
/* ===== Animación de avatar: aleatoria por prefijo y orden ===== */
(()=>{
...
/* ===== Metas y progreso: 2 incrementos y reinicio + confeti liviano ===== */
(()=>{
  ...
  const INITIAL_WIDTH = 60;   // % del contenedor
  const INITIAL_GOAL  = 3000; // pasos
...
/* ===== Duelos 1v1 + Cooperativo (UI animada) ===== */
(()=>{
  ...
  function setActive(scene){
    [scene1v1, sceneCoop].forEach(s => s.classList.remove('active'));
    scene.classList.add('active');
  }

  const fmt = n => n.toLocaleString('es-AR');

  // 1v1: barras animadas siempre
  async function play1v1(){
    ...
    p1Txt.textContent = '0';
    p2Txt.textContent = '0';
    void p1Fill.offsetWidth;
    void p2Fill.offsetWidth;
  ...
  // Cooperativo: mini-barras individuales + HP a 0
  async function playCoop(){
    ...
    coopMembers.forEach(m=>{
      ...
      const v = 25 + Math.random()*70; // 25–95
      bar.style.width = '0%';
...
/* ===== Ranking dinámico con sobrepasos (FLIP) ===== */
(()=>{
  ...
  let items = [...list.children].map((el, idx) => ({
    ...
    vel: (Math.random()*20+10) * (Math.random()<.5?-1:1) // px/s ~ delta pasos
  }));

  const clamp = (v,min,max)=>Math.max(min,Math.min(max,v));
  const fmt   = n => n.toLocaleString('es-AR');

  // Actualización suave de puntos
  function updatePoints(dt){
    items.forEach(it=>{
      // Ruido + leve sesgo para que haya cambios y cruces
      const jitter = (Math.random()-0.5)*30;
      it.points += (it.vel*dt*0.06) + jitter;
      // Rebote suave entre 2000 y 7000
  ...
  // FLIP: animar de su posición anterior a la nueva
  function reorderWithFLIP(){
    // 1) First: posiciones actuales
    const first = new Map(items.map(it => [it.el, it.el.getBoundingClientRect().top]));

    // 2) Orden objetivo (más puntos arriba)
    ...
    // 4) Last: posiciones nuevas y aplicar transform inverso
    items.forEach((it,i)=>{
      const lastTop = it.el.getBoundingClientRect().top;
      const inv = first.get(it.el) - lastTop; // delta Y
      it.el.style.transform = `translateY(${inv}px)`;
      it.el.style.transition = 'transform 0s';

      // Marcadores de sobrepaso
      const from = prevIndex.get(it.el);
      if(from != null){
        if(i < from){ // subió posiciones
          it.el.classList.remove('overtaken');
          it.el.classList.add('overtake');
          setTimeout(()=> it.el.classList.remove('overtake'), 700);
        }else if(i > from){ // bajó
          it.el.classList.remove('overtake');
          it.el.classList.add('overtaken');
          setTimeout(()=> it.el.classList.remove('overtaken'), 700);
        }
      }

      // 5) Play
      requestAnimationFrame(()=>{
        it.el.style.transition = 'transform 520ms cubic-bezier(.2,.7,.2,1)';
  ...
  let accum = 0;
  function loop(now){
    const dt = clamp((now - last)/16.67, 0, 3); // ~frames
    last = now;

    updatePoints(dt);
    accum += now - (now - dt*16.67);

    // Reordenar ~cada 700ms para que el cruce se note
    if(accum >= 700){
      accum = 0;
      reorderWithFLIP();
    }
  ...
  // Primer FLIP para “congelar” estado inicial
  reorderWithFLIP();
  requestAnimationFrame(loop);
})();