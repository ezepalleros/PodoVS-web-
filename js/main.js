/* ========= 1) Carrusel "Un vistazo a la app" (Bootstrap) ========= */
(() => {
  const el = document.getElementById('screensCarousel');
  if (!el) return;
  ...
  const counter = document.querySelector('#screens [data-role="screen-counter"]');
  ...
  const initial = items.findIndex(it => it.classList.contains('active'));
  update(initial >= 0 ? initial : 0);

  el.addEventListener('slid.bs.carousel', (ev) => {
    if (typeof ev.to === 'number') update(ev.to);
  });
})();
...
/* ========= 3) Header blanco: sombra + link activo ========= */
(() => {
...
(() => {
  ...
  const preload = urls => Promise.all(
    [...new Set(urls)].map(u => new Promise(res => {
      const i = new Image();
      i.onload = res; i.onerror = res; i.src = u;
    }))
  );
  ...
  function wear(slot, src) {
    ...
    requestAnimationFrame(() => {
      img.classList.add('show', 'popping');
      setTimeout(() => img.classList.remove('popping'), 280);
    });
  }
  ...
  (async function playLoop() {
    ...
    while (true) {
      ...
      for (const slot of order1) {
        if (look1[slot]) wear(slot, look1[slot]);
      ...
      for (const slot of order2) {
        if (look2[slot]) wear(slot, look2[slot]);
        await new Promise(r => setTimeout(r, 230 + Math.random() * 150));
      }
...
(() => {
  ...
  const updateLabel = () =>
    text.textContent = `${fmt(Math.min(current, stepsGoal))} / ${fmt(stepsGoal)} pasos`;
...
(() => {
  ...
  (async function loop() {
    ['img/stickrunning.png', 'img/stickrunning-vs.png', 'img/pixelmonster.png']
...
/* ========= 7) Ranking dinámico (FLIP) ========= */
(() => {
  const stage = document.querySelector('.ranking-stage');
  if (!stage) return;
  const list = stage.querySelector('.ranking-list');

  let items = [...list.children].map(el => ({
    el,
    points: parseInt(
      el.querySelector('.points').textContent.replace(/\D/g, ''), 10
    ) || 3000,
