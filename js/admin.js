...
if (document.body.dataset.page === 'admin-rooms') {
  (async () => {
    ...
    async function loadEvents() {
      ...
      try {
        ...
        snap.forEach(doc => {
          ...
          tr.innerHTML = `
            <td class="mono-cell"><code class="small">${doc.id}</code></td>
            <td>${escapeHtml(e.ev_title || '—')}</td>
            <td>${tsToDate(e.ev_startAt)}</td>
            <td>${tsToDate(e.ev_endAt)}</td>
    ...
    evForm?.addEventListener('submit', async (e) => {
      ...
      if (!title) {
        showAlert('warning', 'El evento debe tener un título.');
        return;
      }
      if (!startVal || !endVal) {
        showAlert('warning', 'Debés elegir fechas válidas.');
        return;
      }
      ...
      const payload = {
        ...
        ev_startAt: firebase.firestore.Timestamp.fromDate(startDate),
        ev_endAt: firebase.firestore.Timestamp.fromDate(endDate),
      };
